...
func Test_isPrime(t *testing.T) {
	primeTests := []struct {
		...
		{"1", 1, false, "1 is not a prime, by definition!"},
...
func Test_prompt(t *testing.T) {
	...
	// set os.Stdout ot our wwrite pipe
	os.Stdout = w
	...
	// reset os.Stdout of what it was before
	os.Stdout = oldOut
	...
	// perform our test
	if string(out) != "-> " {
		t.Errorf("incorrect propmt: expted -> but got %s", string(out))
	}
}
...
func Test_intro(t *testing.T) {
	...
	if !strings.Contains(string(out), "Enter a whole number") {
		t.Errorf("intro text but got %s", string(out))
	}
}
...
func Test_checkNumbers(t *testing.T) {
	tests := []struct {
		...
		{name: "QUIT", input: "Q", expected: ""},
		{name: "greek", input: "√ç˜ß∆", expected: "Please enter a whole number!"},
	}
